...
unsigned char *cMD2(unsigned char *message, unsigned long message_len, unsigned char *digest)
{
	...
	unsigned long remaining_bytes = 16 - (message_len % 16);
	unsigned long N = message_len + remaining_bytes;
	unsigned long _N = N + 16;
	unsigned char *M = (unsigned char *)malloc(_N);
	unsigned char X[48], C[16], L;
