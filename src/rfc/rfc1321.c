...
uint8_t *MD5(uint8_t *message, uint64_t message_len, uint8_t *digest)
{
	...
	for (uint64_t i = 0; i <= N / 64 - 1; i++)
	{
		...
		// Round 2
		_a5(&A, B, C, D, 1, 5, 17, G_md5, X);
		_a5(&D, A, B, C, 6, 9, 18, G_md5, X);
		_a5(&C, D, A, B, 11, 14, 19, G_md5, X);
		_a5(&B, C, D, A, 0, 20, 20, G_md5, X);

		_a5(&A, B, C, D, 5, 5, 21, G_md5, X);
		_a5(&D, A, B, C, 10, 9, 22, G_md5, X);
		_a5(&C, D, A, B, 15, 14, 23, G_md5, X);
		_a5(&B, C, D, A, 4, 20, 24, G_md5, X);

		_a5(&A, B, C, D, 9, 5, 25, G_md5, X);
		_a5(&D, A, B, C, 14, 9, 26, G_md5, X);
		_a5(&C, D, A, B, 3, 14, 27, G_md5, X);
		_a5(&B, C, D, A, 8, 20, 28, G_md5, X);

		_a5(&A, B, C, D, 13, 5, 29, G_md5, X);
		_a5(&D, A, B, C, 2, 9, 30, G_md5, X);
		_a5(&C, D, A, B, 7, 14, 31, G_md5, X);
		_a5(&B, C, D, A, 12, 20, 32, G_md5, X);
